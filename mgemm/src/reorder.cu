...
template <int bdx, int GROUP_SIZE, int HIDDEN_DIM>
__global__ void reorder_quantize_mxfp8_kernel(
  bf16_t *input,
  int16_t *reorder_index,
  uint8_t *f4out,
  uint8_t *f6out,
  ...
  int row_id = blockIdx.x;
  input = input + row_id * HIDDEN_DIM;
  f4out = f4out + row_id * (GROUP_SIZE * GROUP_NUM(KN));
  f6out = f6out + row_id * (GROUP_SIZE * GROUP_NUM(KS));
  f8out = f8out + row_id * (GROUP_SIZE * GROUP_NUM(KO));
  ...
  // fp4_t* input_frag_fp6 = reinterpret_cast<fp4_t*>(input_frag);
  // fp4_t* input_frag_fp4 = reinterpret_cast<fp4_t*>(input_frag);
  fp8_t* input_frag_fp4 = reinterpret_cast<fp8_t*>(input_frag);
  fp8_t* input_frag_fp6 = reinterpret_cast<fp8_t*>(input_frag);
  fp8_t* input_frag_fp8 = reinterpret_cast<fp8_t*>(input_frag);
  ...
  for(int i = 0; i < elements_per_thread; i += 2){
    ...
    if(tid >= bdx - GROUP_NUM(KO)){
      ...
      input_frag_fp6[i + 0] = converterO(result_0);
      input_frag_fp6[i + 1] = converterO(result_1);
    }
    else {
      input_frag_fp4[i + 0] = converterO(result_0);
      input_frag_fp4[i + 1] = converterO(result_1);
    }
  }
  // Store frag out to global memory
  if(tid >= bdx - GROUP_NUM(KO)){
    ...
    // Store fp8_t quantized result
    float4* f6out_float4 = reinterpret_cast<float4*>(f6out);
    f6out_float4[(tid + GROUP_NUM(KO + KS) - bdx) * 2 + 0] = input_frag_float4[0];
    f6out_float4[(tid + GROUP_NUM(KO + KS) - bdx) * 2 + 1] = input_frag_float4[1];
  }
  else {
    // Store fp8_t quantized result
    float4* f4out_float4 = reinterpret_cast<float4*>(f4out);
    f4out_float4[tid * 2 + 0] = input_frag_float4[0];
    f4out_float4[tid * 2 + 1] = input_frag_float4[1];
  }
}
...
void run_reorder_quantize_x8(
  ...
  Tensor sfas_tensor = cute::make_tensor(sensitive_scale, filter_zeros(sensitive::get_layoutSFA(seq_len, KS)));
  Tensor sfao_tensor = cute::make_tensor(outlier_scale, filter_zeros(outlier::get_layoutSFA(seq_len, KO)));
  reorder_quantize_mxfp8_kernel<hidden_dim / 32, group_size, hidden_dim><<<grids, blocks>>>(
    (bf16_t *)hidden_states,
    (int16_t *)reorder_index,
    (uint8_t *)o_normal,
    (uint8_t *)o_sensitive,
...
template void run_reorder_quantize_x8<32, 4096>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 3584>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 3072>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 5120>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 14336>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 18944>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 11008>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 12288>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 13824>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
...
template void run_reorder_quantize_x8<32, 8192>(
  bf16_t*, int, int16_t*, uint8_t*, uint8_t*, uint8_t*,
  sf_t*, sf_t*, sf_t*, int, int, int
);
