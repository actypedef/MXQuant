...
class QLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        AN, AS, AO, SFAN, SFAS, SFAO = mixedgemm.reorder_quantize_x8(hidden_states, self.q_reorder_index, self.q_proj.p4_num, self.q_proj.p6_num, self.q_proj.p8_num)
        ...
        AN, AS, AO, SFAN, SFAS, SFAO = mixedgemm.reorder_quantize_x8(attn_output, self.o_reorder_index, self.o_proj.p4_num, self.o_proj.p6_num, self.o_proj.p8_num)
        torch.cuda.synchronize()
...
class QLlamaMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        AN, AS, AO, SFAN, SFAS, SFAO = mixedgemm.reorder_quantize_x8(x, self.up_reorder_index, self.up_proj.p4_num, self.up_proj.p6_num, self.up_proj.p8_num)
        torch.cuda.synchronize()
        ...
        AN, AS, AO, SFAN, SFAS, SFAO = mixedgemm.reorder_quantize_x8(tmpResult, self.down_reorder_index, self.down_proj.p4_num, self.down_proj.p6_num, self.down_proj.p8_num)
        torch.cuda.synchronize()
